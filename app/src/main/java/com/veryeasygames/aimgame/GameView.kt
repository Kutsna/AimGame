...
class GameView @JvmOverloads constructor(
    ...
    private val fourthHoleIdx = IntArray(5) { -1 }
...
// Added 4 more balls
    ...
    fun updateHolePositions(
        ...
        java.util.Arrays.fill(fourthHoleIdx, -1)
        ...
        // —— LEVELS 7–10: Level-6 holes + incremental 90° additions ——
        } else if (currentLevel in 7..10) {
            // 1) Rebuild Level 6’s 9-hole mirror layout
            holes += base
            run {
                for (i in 1..4) {
                    val (fx, fy) = base[i]
            ...
            // 2) Add third hole (+90°) on first N rings: 7→1, 8→2, 9→3, 10→4
            val sector = (Math.PI / 2).toFloat()  // 90°
            val extra = (currentLevel - 6).coerceIn(0, 4)
            for (ringIdx in 1..extra) {
                ...
                val a3     = angle0 + sector      // +90°
                holes += (centerX + cos(a3) * r to centerY + sin(a3) * r)
            }
        ...
        // —— LEVELS 16–19: Level-6 holes + incremental 90° additions ——
        } else if (currentLevel in 16..19) {
            // 1) Rebuild Level 6’s 9-hole mirror layout
            holes += base
            run {
                for (i in 1..4) {
            ...
            // 2) Add third hole (+90°) on first N rings: 16→1, 17→2, 18→3, 19→4
            val sector = (Math.PI / 2).toFloat()  // 90°
            val extra = (currentLevel - 15).coerceIn(0, 4)
            for (ringIdx in 1..extra) {
                val (fx, fy) = base[ringIdx]
                val px = if (fx <= 1f) fx * width else fx
                val py = if (fy <= 1f) fy * height else fy
        ...
        // —— LEVELS 20–23: base 9, +90° on all rings, +270° cumulatively by ring ——
        } else if (currentLevel in 20..23) {
            // 1) Level-6’s 9-hole layout (base + mirrors)
            holes += base
            run {
                for (i in 1..4) {
                    val (fx, fy) = base[i]
            ...
            // 2) Add the 3rd hole (+90°) on ALL rings
            val sector = (Math.PI / 2).toFloat() // 90°
            for (ringIdx in 1..4) {
                val (fx, fy) = base[ringIdx]
                val px = if (fx <= 1f) fx * width else fx
            ...
            val addCount = (currentLevel - 19).coerceIn(1, 4) // 20->1, 21->2, 22->3, 23->4
            for (ringIdx in 1..addCount) {
                val (fx, fy) = base[ringIdx]
                val px = if (fx <= 1f) fx * width else fx
                val py = if (fy <= 1f) fy * height else fy
                val r      = hypot(px - centerX, py - centerY)
                val angle0 = atan2(py - centerY, px - centerX)
                val a4     = angle0 + 3f * sector  // +270° (avoid the 180° mirror duplicate)
                val hx     = centerX + cos(a4) * r
                val hy     = centerY + sin(a4) * r
                fourthHoleIdx[ringIdx] = holes.size  // remember index for snapping
                holes += (hx to hy)
            }
        }
    ...
    init {
        ...
        soundIds["coins_m"] = soundPool.load(context, R.raw.coins_m, 1)
        soundIds["ballout"] = soundPool.load(context, R.raw.ballout, 1)
        soundIds["tashti"] = soundPool.load(context, R.raw.tashti, 1)
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        // val bannerHeightPx = if (removeAds) 0f else AdSize.LARGE_BANNER.getHeightInPixels(context).toFloat()
        ...
        if (currentLevel == 1 && !balls[0].placed) {
            ...
            val fraction = if (baseDp < 500f) {
                0.035f   // phones & small tablets: 5% of baseDp
            } else {
                0.025f  // large tablets: only 2.5%
            }
        ...
        if (!startDialogShowing && !levelFailed && !levelCompleted) {
            ...
            // 2) If expanded, draw the allOptions menu
            if (optionsExpanded) {
        ...
        if (!levelCompleted && !levelFailed) {
            ...
            // Draw the TIME bar only if the user hasn’t removed the time-limit
            if (!removeTimeLimit && currentLevel >= 11 && firstSnapOccurred) {       // was showTimer
    ...
    private fun updateGameState(canvas: Canvas) {
        val floorY = height - AdSize.LARGE_BANNER.getHeightInPixels(context).toFloat()
        ...
        val sens = when (currentLevel) {
            1 -> 0.2f * scaleFactor; // one-by-one balls, snap-out not possible
            ...
            7,11 -> 0.7f * scaleFactor; // 10-14 balls, unsnap possible
            else -> 1.0f * scaleFactor
        }
        balls.forEach { ball ->
            ...
            val elasticity = 0.92f
            val minBounce = 0.8f  // pixels/frame; prevents “dead” bounces

// left/right walls
            if (ball.x < ball.radius) {
                ball.x = ball.radius
                ball.vx = -ball.vx * elasticity
                if (abs(ball.vx) < minBounce) ball.vx = -minBounce
...
// top wall
            if (ball.y < ball.radius) {
                ...
                if (abs(ball.vy) < minBounce) ball.vy =  minBounce
            }
// bottom wall — use the gameplay floor, not full height
    ...
    private fun playBallSound(ball: Ball) {
        ...
        if (soundEnabled) {
            when (index) {
                ...
                13 -> playSoundThrottled("ball2")
                14 -> playSoundThrottled("ball3")
                15-> playSoundThrottled("ball4")
                16 -> playSoundThrottled("ball5")
            }
        }
    }
    ...
    private fun playBellSound(ball: Ball) {
        ...
        if (soundEnabled) {
            ...
            when (idx) {
                ...
                13 -> soundPool.play(soundIds["bell_1"] ?: 0, 1f, 1f, 0, 0, 1f)
                14 -> soundPool.play(soundIds["bell_2"] ?: 0, 1f, 1f, 0, 0, 1f)
                15 -> soundPool.play(soundIds["bell_3"] ?: 0, 1f, 1f, 0, 0, 1f)
                16 -> soundPool.play(soundIds["bell_4"] ?: 0, 1f, 1f, 0, 0, 1f)
    ...
    // Tablet-friendly menu adapter: 2× text on tablets; centered + white;
// on phones, rows are ~half height (tight spacing).
    private fun menuAdapter(items: List<String>): ArrayAdapter<String> {
        val isTablet = resources.configuration.smallestScreenWidthDp >= 600
        fun dp(v: Int) = (v * resources.displayMetrics.density).toInt()

        return object : ArrayAdapter<String>(context, android.R.layout.simple_list_item_1, items) {
            override fun getView(position: Int, convertView: View?, parent: ViewGroup): View {
                val tv = super.getView(position, convertView, parent) as TextView

                // style
                tv.textSize = if (isTablet) 32f else 14f
                ...
                if (!isTablet) {
                    // Force a shorter row height + small padding (≈ half the default)
                    val rowH = dp(36) // tweak to taste (32–40dp)
    ...
    private fun allOptions() {
        ...
        val adapter = menuAdapter(items.toList())

        // 2) Create the dialog with an adapter callback
        val dialog = AlertDialog.Builder(context)
            // .setTitle("Options")   // <— added
            .setAdapter(adapter) { dialogInterface, which ->
                ...
                analytics.logEvent(FirebaseAnalytics.Event.SELECT_ITEM) {
                    param(FirebaseAnalytics.Param.ITEM_ID, which.toString())
                    param(FirebaseAnalytics.Param.ITEM_NAME, items[which])
                    param(FirebaseAnalytics.Param.CONTENT_TYPE, "all_options_menu")
                }
                ...
                when (which) {
                    ...
                    3 -> {                         // "Flat"
                        orientationOffset = 0    // Set Flat mode (no Tilt)
                    ...
                    4 -> billingManager.getRefundInfo { info ->
                        // After this call, flags are already flipped if refunded.
                        (context as? Activity)?.runOnUiThread {
                            //if (info.ownedNow) {
                            //    Toast.makeText(context, "Premium already unlocked", Toast.LENGTH_SHORT).show()
                            //} else {
                            billingManager.showRemoveOptionsDialog()
                        }//}
                    }
                    5 -> showPrivacyPolicy()
                    6 -> showTermsOfUse()
        ...
        if (resources.configuration.smallestScreenWidthDp < 600) {
            ...
            dialog.listView?.apply {
                dividerHeight = dp(- 4)      // smaller gaps between rows
    ...
    private fun showInstructionDialog() {
        ...
        tvTitle.setTextSize(TypedValue.COMPLEX_UNIT_SP, 18f * scaleFactor)
        tvMessage.setTextSize(TypedValue.COMPLEX_UNIT_SP, 12f * scaleFactor)
