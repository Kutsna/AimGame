        ...
        val builder = AlertDialog.Builder(activity)
            ...
            .setItems(items.toTypedArray()) { _, which ->
                when (items[which]) {
                    ...
                    "Remove Timer from Lv11: ${getPrice(SKU_REMOVE_TIMER)}" -> launchPurchaseFlow(
                        SKU_REMOVE_TIMER
                    )
            ...
            for (purchase in purchases) {
                if (purchase.purchaseState == Purchase.PurchaseState.PURCHASED) {
                    ...
                    when (purchase.products.firstOrNull()) {
                        SKU_REMOVE_ADS   -> removeAds = true
                        SKU_REMOVE_TIMER -> removeTimeLimit = true
                        // SKU_REMOVE_ALL  -> removeAds = true; removeTimeLimit = true
                    }

                    // 3) Push into your GameView & hide ads
                    applyEntitlements()

                    // 4) If you still need to clear your Activity’s banners/runnable:
                    (activity as? MainActivity)?.hideAllAds()
