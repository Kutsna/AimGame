...
class GameView @JvmOverloads constructor(
    ...
    private val optionsList = listOf("Main Menu", "Set Tilt", "Set Flat") // New Game omitted
    ...
    override fun onTouchEvent(event: MotionEvent): Boolean {
        ...
        when (event.action) {
            MotionEvent.ACTION_DOWN -> {
                // If dropdown open, see if they pressed an item
                if (optionsExpanded) {
                    val itemHeight = 80f * scaleFactor
                    val spacing    = 8f  * scaleFactor
                    var topY       = (optionsButtonRect.bottom + spacing) // * scaleFactor
                    ...
                    for (i in optionsList.indices) {
                        ...
                        if (itemRect.contains(x, y)) {
                            pressedDropdownIndex = i
                            playSound("pwa")
                            invalidate()       // redraw so onDraw shows pressed state
                            return true
                        }
                        topY += itemHeight + spacing
                    }
                }
                // Otherwise, see if they pressed the main Options button
                if (optionsButtonRect.contains(x, y) && !startDialogShowing) {
            ...
            MotionEvent.ACTION_UP, MotionEvent.ACTION_CANCEL -> {
                // If they had a dropdown-item pressed, fire it now
                if (pressedDropdownIndex >= 0) {
                    handleOptionSelection(pressedDropdownIndex)
                    pressedDropdownIndex = -1
                    optionsExpanded = false
                    invalidate()
                    return true
                }
                // If they had the main Options pressed, toggle dropdown
        ...
        if (event.action == MotionEvent.ACTION_DOWN) {
            ...
            if (optionsExpanded) {
                val itemHeight = 90f * scaleFactor
                val spacing    = 12f * scaleFactor
    ...
    fun animateHiScoreBounce() {
        ValueAnimator.ofFloat(1f * scaleFactor, 3f * scaleFactor, 1f * scaleFactor).apply {
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        if (currentLevel == 1 && !balls[0].placed) {
            val fullText = "Tilt your phone/tablet to\ndrop the ball into hole.\n\nTo adjust position hold your phone\n" +
                    "    comfortably tilted - use Options→Tilt."
        ...
        if (showHoles) {
            ...
            val holeRadius = 12f * scaleFactor
        ...
        if (!startDialogShowing && !levelFailed && !levelCompleted) {
            // 1) Metrics & sizing
            val margin    = 16f * scaleFactor                                    // margin to screen edges
            ...
            canvas.drawRoundRect(optionsButtonRect, 12f * scaleFactor, 12f * scaleFactor, bggPaint)
            // your existing textSize
            val textLabel = "Options" // Ξ"
            ...
            canvas.drawRoundRect(optionsButtonRect, 12f * scaleFactor, 12f * scaleFactor, bgPaint)
        ...
        if (!levelCompleted && !levelFailed) {
            val barMargin = 20f * scaleFactor
            val bottomMargin = 20f * scaleFactor
            val barHeight = 35f * 2.8f * scaleFactor     // same as before, for consistency
...
/*
            // Draw the LEVEL bar rectangle
            canvas.drawRect(
                barMargin,
                levelBarTop,
                barMargin + filledLevelWidth,
                levelBarTop + barHeight,
                levelBarPaint
            )

            // Draw “Level: N” text over the LEVEL bar (same as before)
            ...
            canvas.drawText(
                "Level: $currentLevel",
                barMargin + 20f,
                levelBarTop + barHeight - 20f,
                levelTextStrokePaint
            )
            canvas.drawText(
                "Level: $currentLevel",
                barMargin + 20f,
...
*/
            ...
            // ─── 3) Compute elapsed‐seconds ONLY if firstSnapOccurred == true ───────────────
            ...
            // ─── 4) Clamp to a maximum of 90s for the fill‐percentage ────────────
            val clampedElapsed = elapsedSeconds.coerceAtMost(90f)
            val timeFraction  = (clampedElapsed / 90f).coerceIn(0f, 1f)
            ...
            val magentaColor = Color.RED

            val timeBarColor: Int = when {
                elapsedSeconds <= 10f -> {
                    // 0‒10 sec: always pure green
                    greenColor
                }
                elapsedSeconds <= 30f -> {
                    // 10‒30 transition: green→yellow
                    val localFrac = (elapsedSeconds - 10f) / (30f - 10f)
                    interpolateColor(greenColor, yellowColor, localFrac)
                }
                elapsedSeconds <= 60f -> {
                    // 30‒60 transition: yellow→orange
                    val localFrac = (elapsedSeconds - 30f) / (60f - 30f)
                    interpolateColor(yellowColor, orangeColor, localFrac)
                }
                elapsedSeconds <= 90f -> {
                    // 60‒90 transition: orange→magenta
                    val localFrac = (elapsedSeconds - 60f) / (90f - 60f)
                    interpolateColor(orangeColor, magentaColor, localFrac)
                }
                else -> {
                    // any time above 90s: solid magenta (fill is full by now anyway)
                    magentaColor
                }
            }
            ...
            if (showTimer) {
                canvas.drawText("Time: $levelTimer", barMargin + 20f * scaleFactor, timeBarTop + barHeight - 20f * scaleFactor, timeTextStrokePaint)
                canvas.drawText("Time: $levelTimer", barMargin + 20f * scaleFactor, timeBarTop + barHeight - 20f * scaleFactor, timeTextFillPaint)
            }
            canvas.restore()
        }
        ...
        if (!timerPaused && !levelCompleted && !levelFailed && !startDialogShowing) {
            // Schedule the next frame in ~33ms (≈30 fps)
            handler.postDelayed({ invalidate() }, 1L)
        }}
    ...
    private fun updateGameState(canvas: Canvas) {
        ...
        // 1) Timer
        // 1) Timer (hold at zero until firstSnapOccurred == true)
        val elapsedSeconds: Float = if (!firstSnapOccurred) {
            0f
        } else {
            val rawElapsedMs = if (timerPaused) pausedElapsed else (now - levelStartTime)
        ...
        levelTimer = elapsedSeconds.toInt()
        ...
        // 8) Failure
        if (levelTimer<=-1 && !levelCompleted && !levelFailed) {
                ...
                failureDialogRunnable = Runnable {
                    ...
                    dialog.setOnShowListener {
                        dialog.getButton(AlertDialog.BUTTON_POSITIVE)?.apply {
                            ...
                            setTextSize(20f * scaleFactor)
                        }
                        dialog.getButton(AlertDialog.BUTTON_NEGATIVE)?.apply {
                            setTextColor(Color.YELLOW)
                            setShadowLayer(2f * scaleFactor, 4f * scaleFactor, 4f * scaleFactor, Color.BLACK)
                            setTextSize(20f * scaleFactor)
                        }
                    }
                    dialog.show()
                ...
                handler.postDelayed({
                    val dialog = AlertDialog.Builder(context)
                        ...
                        .setMessage(Html.fromHtml("<font color='green'>You have reached the highest level.\n" +
                                "Would you like to restart the Game?</font>", Html.FROM_HTML_MODE_LEGACY))
    ...
    private fun showStartDialog() {
        ...
        handler.post {
            ...
            if (prefs.contains("overallScore") && prefs.contains("currentLevel")) {
                ...
                btnPos.apply {
                    text = "Continue last session at Score: $savedScore, Level: $savedLevel"
    ...
    private fun allOptions() {
        ...
        dialog.window?.apply {
            // use the new 90%-opaque yellow‐framed drawable
    ...
    private fun showPrivacyPolicy() = showLongTextDialog(context, "Privacy Policy", R.string.privacy_policy)
    private fun showTermsOfUse()    = showLongTextDialog(context, "Terms of Use", R.string.terms_of_use)
    private fun showHowToPlay()     = showLongTextDialog(context, "How to Play", R.string.how_to_play)
